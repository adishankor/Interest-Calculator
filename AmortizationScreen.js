...
import {
  View, ScrollView, Text, TouchableOpacity, StyleSheet, Alert, FlatList
} from 'react-native';
...
const AmortizationScreen = ({ navigation, route }) => {
  ...
  const handleCalculate = () => {
    ...
    if (!r) {
      Alert.alert('Invalid Input', 'Please enter valid positive values.');
      return;
    }
    setResult(r);
    setShowAll(false);
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#92400E22', '#F59E0B22']} style={styles.infoBanner}>
          ...
          <Text style={styles.infoText}>
            See exactly how much of each monthly payment goes to principal vs interest, and your remaining balance every month.
          </Text>
        </LinearGradient>
          ...
          <>
            ...
            {displayedRows.map((row, i) => (
              <View
                ...
                <Text style={[styles.td, { flex: 1.3 }, { color: colors.teal }]}>
                  {formatCurrency(row.principal).replace('৳', '')}
                </Text>
                <Text style={[styles.td, { flex: 1.3 }, { color: colors.coral }]}>
                  {formatCurrency(row.interest).replace('৳', '')}
                </Text>
                <Text style={[styles.td, { flex: 1.4 }, { color: colors.textPrimary }]}>
...
const styles = StyleSheet.create({
  ...
  tableHeader: {
    ...
    paddingHorizontal: spacing.sm, backgroundColor: colors.bgSurface,
  ...
  tableRow: {
    ...
    paddingHorizontal: spacing.sm, borderRadius: 4,
  },
