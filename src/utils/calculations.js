...
export const calculateDPS = ({ monthlyDeposit, annualRate, months, isCompound }) => {
  ...
  return { monthlyDeposit: P, totalDeposit, totalInterest, maturityAmount, interestRatio: (totalInterest / totalDeposit) * 100 };
};
...
export const calculateFDR = ({ principal, annualRate, years, isCompound, compoundFreq }) => {
  ...
  return { principal: P, totalInterest, maturityAmount, effectiveRate: (totalInterest / P / t) * 100, interestRatio: (totalInterest / P) * 100 };
};
...
export const calculateEMI = ({ principal, annualRate, months }) => {
  ...
  return { emi, totalPayment, totalInterest, principal: P, interestPercent: (totalInterest / totalPayment) * 100, principalPercent: (P / totalPayment) * 100 };
};
...
export const calculateAmortization = ({ principal, annualRate, months }) => {
  ...
  for (let month = 1; month <= n; month++) {
    ...
    schedule.push({ month, emi, principal: principalPayment, interest: interestPayment, balance, totalInterestPaid });
  }
  return { emi, schedule, totalPayment: emi * n, totalInterest: totalInterestPaid, principal: P };
};

export const calculateGoalPlanner = ({ targetAmount, currentSavings, years, annualRate }) => {
  ...
  const monthlySIP = remainingGoal > 0 ? (remainingGoal * monthlyRate) / (Math.pow(1 + monthlyRate, n) - 1) : 0;
  ...
  return { targetAmount: FV, currentSavings: CS, fvCurrentSavings, remainingGoal, monthlySIP, lumpSum, totalSIPInvestment, sipInterestEarned: Math.max(0, sipInterestEarned), years: t, months: n, annualRate: r };
};
