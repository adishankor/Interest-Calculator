...
const EMIScreen = ({ navigation }) => {
  ...
  const handleCalculate = () => {
    ...
    if (!r) { Alert.alert('Invalid Input', 'Please enter valid values for all fields.'); return; }
    setResult(r);
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#6D28D922', '#8B5CF622']} style={styles.infoBanner}>
          ...
          <Text style={styles.infoText}>Calculate your monthly loan repayment amount based on loan amount, interest rate, and tenure.</Text>
        </LinearGradient>
        <CustomInput label="Loan Amount" value={principal} onChangeText={setPrincipal} prefix="৳" placeholder="e.g. 500000" />
        <CustomInput label="Annual Interest Rate" value={annualRate} onChangeText={setAnnualRate} suffix="% p.a." placeholder="e.g. 10.5" />
        <CustomInput label="Loan Tenure" value={months} onChangeText={setMonths} suffix="months" placeholder="e.g. 60" hint="12=1yr, 60=5yrs, 240=20yrs" />
        ...
        {result && (
          <>
            <ResultCard highlightValue={formatCurrency(result.emi)} highlightLabel="Monthly EMI" gradColors={['#6D28D9', '#8B5CF6']}>
              ...
              <RatioBar label1="Principal" label2="Interest" percent1={result.principalPercent} color1={colors.purple} color2={colors.coral} />
            </ResultCard>
            <TouchableOpacity style={styles.amortBtn} onPress={() => navigation.navigate('AmortizationCalculator', { principal, annualRate, months })} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  infoBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm, padding: spacing.md, borderRadius: radius.md, marginBottom: spacing.lg, borderWidth: 1, borderColor: colors.purple + '30' },
  ...
  amortBtn: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginTop: spacing.md, padding: spacing.md, backgroundColor: colors.bgCard, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.gold + '40', justifyContent: 'center' },
